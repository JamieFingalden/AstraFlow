You are now my backend engineer.  
Please generate a **complete, production-level Go backend** using **Go 1.22**, **Gin**, **GORM**, and **MySQL 8.0**.

This backend is for an **AI-powered reimbursement & bill management platform**, supporting:

- Multi-tenant mode (company accounts)
- Individual mode (self-use users)
- Soft linking between tables (NO foreign key constraints)
- File upload
- OCR result storage (OCR is external Flask service)
- Bill management
- JWT authentication
- RESTful API layering (Controller → Service → Repository)
- Full CRUD for all main tables
- Extensible architecture

==================================================
1. DATABASE SCHEMA (SQL V2) — USE EXACTLY
==================================================
...
*** Soft-link rule: NO foreign key constraints. All linking handled in service logic only. ***
...
- JWT auth
- Tenant isolation:
  - individual user: no tenant_id required
  - tenant mode: tenant_id required

AUTH:
- login (JWT)
- signup (admin / member / individual)
- tenant admin can create users

CRUD:
Generate FULL CRUD for:
- tenants
- users
- upload_files
- ocr_results
...
POST /api/ocr/parse
- accept file_id
- call external Flask OCR service (placeholder only)
- store result in ocr_results
...
1. Fully runnable Go code
2. All files complete (no pseudo code)
3. Valid go.mod
4. All models match SQL EXACTLY
5. Controllers, services, repositories, routes, middleware
6. Config + logger
7. Example JSON responses
8. Directory-structured output format

==================================================
5. FORMAT REQUIREMENTS
==================================================

- Provide the entire backend codebase
- Organized by folder → file → code
